...
from models.sales import Sales as SalesModel

from schema import SalesBase, SalesInDB, SalesInfo, SalesCreate
...
summary="single sale",
status_code=200)
def view_productSales(saleID:int, db: Session = Depends(get_db)):
   
    sale = db.query(Sales.id, Products.name, func.sum(Sales.quantity).label("Quantity"), func.sum((Products.sp-Products.bp)*Sales.quantity).label("Profit")).join(Sales, Products.id == Sales.product_id).group_by(Products.name, Products.id, Sales.id).filter(SalesModel.id == saleID).all()
    for x in sale:
